...
section "COMPREHENSIVE CONTRIBUTOR TESTS (password: user123)"
echo ""

SIH_ID="01e331eb-0e30-4ada-adf9-fc5824f82675"
STUDENT_001_ID="300b39cd-422b-40da-aacf-87573457aa26"
STUDENT_002_ID="58e683ee-f25a-42ad-89db-891af24bfcdb"
STUDENT_003_ID="1ed87d8a-b2d5-4128-989d-6a60b86a02f6"
...
TOKEN_001=$(curl -s -X POST "$BASE_URL/auth/login" \
  ...
  -d '{"email":"23bsm001@iiitdmj.ac.in","password":"user123"}' | jq -r '.data.access_token')
...
# Test 1: Add 23bsm004
section "Test 1: Owner (23bsm001) adds 23bsm004 to SIH"
...
[ "$NEW_COUNT" -gt "$COUNT" ] && pass "Count increased: $COUNT -> $NEW_COUNT" || info "Count: $NEW_COUNT"

# Test 2: 23bsm004 can see SIH
section "Test 2: 23bsm004 sees SIH in workspace"
TOKEN_004=$(curl -s -X POST "$BASE_URL/auth/login" \
  -H "Content-Type: application/json" \
  -d '{"email":"23bsm004@iiitdmj.ac.in","password":"user123"}' | jq -r '.data.access_token')
[ -n "$TOKEN_004" ] && [ "$TOKEN_004" != "null" ] && pass "Logged in as 23bsm004" || { fail "Login failed"; }
...
[ "$HAS" -gt 0 ] && pass "23bsm004 sees SIH!" || fail "23bsm004 does NOT see SIH (BUG!)"
...
REM=$(curl -s -X DELETE "$BASE_URL/student/achievements/contributors" \
  ...
  -H "Content-Type: application/json" \
  -d "{\"achievement_id\":\"$SIH_ID\",\"student_id\":\"$STUDENT_004_ID\"}")
echo "$REM" | jq -e '.success' > /dev/null && pass "Removed 23bsm004" || fail "Failed"
...
# Test 5: Non-owner cannot remove
section "Test 5: Non-owner cannot remove (security)"
BAD_REM=$(curl -s -X DELETE "$BASE_URL/student/achievements/contributors" \
  -H "Authorization: Bearer $TOKEN_004" \
  -H "Content-Type: application/json" \
  -d "{\"achievement_id\":\"$SIH_ID\",\"student_id\":\"$STUDENT_001_ID\"}")
echo "$BAD_REM" | jq -e '.error' > /dev/null && pass "Non-owner blocked" || fail "SECURITY ISSUE!"

# Test 6: Search students
section "Test 6: Search students"
...
# Test 7: 23bsm001 workspace has SIH
section "Test 7: 23bsm001 workspace has SIH"
HAS_001=$(echo "$WORK" | jq "[.data.linked_records.achievement_members[]? | select(.achievement_id == \"$SIH_ID\")] | length")
[ "$HAS_001" -gt 0 ] && pass "23bsm001 sees SIH!" || fail "23bsm001 does NOT see SIH"

# Test 8: Add 23bsm003
section "Test 8: Owner adds 23bsm003"
ADD3=$(curl -s -X POST "$BASE_URL/student/achievements/contributors" \
  -H "Authorization: Bearer $TOKEN_001" \
  -H "Content-Type: application/json" \
  -d "{\"achievement_id\":\"$SIH_ID\",\"student_id\":\"$STUDENT_003_ID\"}")
echo "$ADD3" | jq -e '.success' > /dev/null && pass "Added 23bsm003" || fail "Failed"

# Test 9: 23bsm003 sees SIH
section "Test 9: 23bsm003 sees SIH in workspace"
TOKEN_003=$(curl -s -X POST "$BASE_URL/auth/login" \
  -H "Content-Type: application/json" \
  -d '{"email":"23bsm003@iiitdmj.ac.in","password":"user123"}' | jq -r '.data.access_token')
WORK3=$(curl -s -X GET "$BASE_URL/student/me" -H "Authorization: Bearer $TOKEN_003")
HAS3=$(echo "$WORK3" | jq "[.data.linked_records.achievement_members[]? | select(.achievement_id == \"$SIH_ID\")] | length")
[ "$HAS3" -gt 0 ] && pass "23bsm003 sees SIH!" || fail "23bsm003 does NOT see SIH (BUG!)"
